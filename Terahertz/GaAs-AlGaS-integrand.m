...
eps1 =  -18.295 - 1i*0.48085; % Upper layer - GaAs
eps2 = -18.295 + 1i*0.48085 ; % Lower Layer - AlGaAs
...
k_silver = omega * sqrt(mu_0*eps_0*eps1); % propagation constant of silver
%%
...
kxx = horzcat(linspace(0*k_air,.5*k_air,len/4),...
    ...
    linspace(1.5*k_air + 1e-6, 1e3*k_air,len/4)); % 1e-6 used to avoid duplicates in the array
...
hyp_silver_re = +kxx.^2 - real(k_silver^2); % Hyperbolic cruve for silver
