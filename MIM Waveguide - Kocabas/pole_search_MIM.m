...
% ep1 = -22.3 + 1i*3.84; %GaAs layer
% ep2 = 11; % 2DEG layer (from conductivity_epsilon
% ep3 = -22.3 + 1i*3.84; % GaAs layer

ep1 = -20 + 1i*1.4; %GaAs layer
ep2 = 3.7 + 1i*.00246; % 2DEG layer (from conductivity_epsilon
ep3 = -22.3 + 1i*3.84; % GaAs layer
...
k1 = conj(omega*sqrt(mu0*ep0*ep1));
k2 = omega*sqrt(mu0*ep0*ep2);
k3 = conj(omega*sqrt(mu0*ep0*ep3));
%%

lxlim = k2;
uxlim = 25*k2;
num = 30; %Size of the arrays
...
if ~isempty(root)
    ...
    while ~isempty(root)
        vct = abs(root - root(1)) < 1e2; % Minimum spacing between roots.
...
for i = 1 : length(root)
    if abs(real(root(i))) > real(k1)
        h1 = plot((real(root(i))/k2), (imag(root(i))/k2), 'o', 'markersize',5,...
            'color','black',...
            'LineWidth',2,...
            'MarkerFaceColor',Colord(1,:));
    else
        h2 = plot((real(root(i))/k2), (imag(root(i))/k2), 'o', 'markersize',2,...
            'color',Colord(2,:),...
            'LineWidth',2,...
...
%     'MarkerFaceColor',Colord(2,:));
h3 = plot((real(k3)/k2) , (imag(k3)/k2), 's', 'markersize',10,...
    'color',Colord(4,:),...
    'LineWidth',2,...
    'MarkerFaceColor',Colord(4,:));
h3 = plot((real(k1)/k2) , (imag(k1)/k2), 's', 'markersize',10,...
    'color',Colord(4,:),...
    'LineWidth',2,...
    'MarkerFaceColor',Colord(4,:));
...
legend([h1 h2 h3], 'Surface waves','Evanescent modes','Branch point',...
