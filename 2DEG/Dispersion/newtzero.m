...
defaults = {1,30,1e-14};% Initial guess, max iterations, tolerance.
...
LGS = logspace(0,3,520); % Can be altered for wider range or denser search.
...
if ~isempty(root)
    ...
    while ~isempty(root)
        vct = abs(root - root(1))/abs(root(1)) < 1e-6; % Minimum spacing between roots.
