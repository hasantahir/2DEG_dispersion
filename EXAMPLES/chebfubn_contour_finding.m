...
% % f = sqrt(z.^2+beta.^2);
% % f = chebfun2(@(x,y) (x.^2+y.^3+beta.^2).*sin(10*x.*y),[-10,10],[-10,10]);
% % % f = log(z);
% % % f = z.^2;
% % % f = z.^2;
% % figure(1)
...
% % % x = chebfun('x',[-40 40]);
% % % y = chebfun('y',[-40 40]);
% % % f = chebfun2(@(x,y) sqrt((x + 1i*y).^2 - beta^2));
% % f = chebfun2(@(z) (z.^2 - 1).^(1/2),'splitting','on');
% % roots(f)
% % (gradient(f)); % critical points
% % % plot(roots(diff(f,1,2),'all'),'b',LW,1.6), hold on % plot zero contours of f_x
% % % plot(roots(diff(f),'all'),'r') % plot zero contours of f_y
...
% % ff = @(x) sqrt(x.^2 + x.^4);
% % ff = chebfun(ff,'splitting','on');
% % fff = @(x) sqrt(x.^2 - x.^4);
% % fff = chebfun(fff,'splitting','on');
% % F = chebfun2(ff,fff)
% % % ff = @(z) cosh(exp(z)).*(z-.3).*(1+4*z.^2);
% % % roots3 = roots(ff)
% % plot(chebfun(ff,'splitting','on')), hold on
% % % plot(z, 'k-'), axis equal
% 
% 
