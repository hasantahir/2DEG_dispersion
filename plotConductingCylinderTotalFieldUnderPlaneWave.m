function plotConductingCylinderTotalFieldUnderPlaneWave(varargin)
% Plot the total field as a plane wave is scattered by a conducting
...
% Optional:
%
% 'radius'        radius of the conducting cylinder
...
%                 to [-3; 3; -2; 2].
%
% 'resolution'    2x1 resolution of the image (relative to
%                 the background wavelength). Default to
%                 [0.05; 0.05].
%
% 'figure_name'   figure name. Default to [].
%
% 'workspace_name'  workspace name to store the data. Default to [].
%                   
%
%
...
% $Date:: 2011-03-27 13:55:53 -0400 (Sun, 27 Mar 2011) $
    ...
    p.addParamValue('domain_size', [-5; +3; -3; +3], @isvector);
    ...
    for iX = 1:size(x, 1);
        for iY = 1:size(y, 2);
            [Ez(iX,iY), H_rho(iX,iY), H_phi(iX,iY)] = getConductingCylinderFieldUnderPlaneWave(p.Results.radius*wavelength, ...
