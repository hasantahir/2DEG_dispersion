function plotDielectricCylinderTotalFieldUnderLineSource(varargin)
...
% $Date:: 2011-03-27 13:55:53 -0400 (Sun, 27 Mar 2011)    $
    ...
    for iX = 1:size(x, 1);
        for iY = 1:size(y, 2);
            [Ez(iX,iY), H_rho(iX,iY), H_phi(iX,iY)] = getDielectricCylinderFieldUnderLineSource(p.Results.radius*wavelength, ...
                                                              p.Results.cylinder, ...
    ...
    for iX = 1:size(x,1)
        for iY = 1:size(y,2)
            ...
            if (norm(sensor_location) > p.Results.radius) % outside the dielectric cylinder
